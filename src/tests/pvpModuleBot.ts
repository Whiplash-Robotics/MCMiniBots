...
bot.on("spawn", () => {
  ...
  bot.chat("/tp @s 30 56 0"); // Use @s for self
  const straight = new Vec3(30, 58, 1);
  bot.lookAt(straight);
  bot.chat("/clear @s");
  bot.chat("/effect give @s minecraft:hunger 1 255");
  bot.chat("/give @s minecraft:golden_carrot");
...
async function shootAtTarget(target: any) {
