...
export function scanFile(filepath, allowedFilepath) {
    ...
    if (!filePath) {
        console.error(`${colors.red}Error: Please provide a file path to check.${colors.reset}`);
        return 2;
    }
    ...
    let allowed_imports = [];
    try {
        allowed_imports = JSON.parse(fs.readFileSync(allowedFilepath, 'utf-8')).allowed;
    } catch (error) {
        console.error(`${colors.yellow}Warning: Could not read './allowed.json'. No imports will be allowed.${colors.reset}`);
    }


    console.log(`${colors.white}🔍 Checking ${colors.cyan}${filePath}${colors.white}...`);
    console.log(`${colors.white}Allowed imports: ${colors.yellow}[${allowed_imports.join(', ')}]${colors.reset}`);
    ...
    traverse(ast, {
        ...
        ImportDeclaration(path) {
            ...
            if (!allowed_imports.includes(importName)) {
                illegalNodes.push({ name: importName, position: node.loc, reason: "Disallowed import" });
            }
        },
        ...
        CallExpression(path) {
            ...
            if (callee.isImport()) {
                if (node.arguments.length === 1 && node.arguments[0].type === 'StringLiteral') {
                    ...
                    if (!allowed_imports.includes(importName)) {
                        illegalNodes.push({ name: `import('${importName}')`, position: node.loc, reason: "Disallowed dynamic import" });
                    }
                } else {
                    illegalNodes.push({ name: 'import()', position: node.loc, reason: "Disallowed dynamic import with non-string argument" });
                }
            ...
            if (callee.isIdentifier({ name: 'eval' })) {
                illegalNodes.push({ name: 'eval()', position: node.loc, reason: "Forbidden call to eval" });
                return;
            }
            ...
            if (isRequireCall) {
                if (node.arguments.length === 1 && node.arguments[0].type === 'StringLiteral') {
                    const importName = node.arguments[0].value;
                    if (!allowed_imports.includes(importName)) {
                        illegalNodes.push({ name: `require('${importName}')`, position: node.loc, reason: "Disallowed require" });
                    }
                } else {
                    illegalNodes.push({ name: 'require()', position: node.loc, reason: "Disallowed dynamic require" });
                }
            }
        },
        ...
        for (const issue of illegalNodes) {
            ...
            for (let i = startLine; i <= endLine; i++) {
                ...
                if (startLine === endLine) {
                    highlight = ' '.repeat(startCol) + '^' + '~'.repeat(Math.max(0, endCol - startCol - 1));
                } else if (i === startLine) {
                    highlight = ' '.repeat(startCol) + '^' + '~'.repeat(Math.max(0, line.length - startCol));
                } else if (i === endLine) {
                    highlight = '^' + '~'.repeat(Math.max(0, endCol - 1));
                } else {
                    highlight = '^' + '~'.repeat(Math.max(0, line.length));
                }
                console.log(' '.repeat(gutterWidth) + colors.red + highlight + colors.reset);
            }
        }
...
scanFile(process.argv[2], "./allowed.json");
