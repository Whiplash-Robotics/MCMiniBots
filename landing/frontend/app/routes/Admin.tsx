...
const Admin: React.FC = () => {
  ...
  const bulkUpdateSubmissions = async (action: 'approve' | 'reject' | 'delete', rejectionReason?: string) => {
    ...
    if (!window.confirm(`Are you sure you want to ${actionText} ${selectedSubmissions.size} submission(s)?`)) {
      return;
    }
    ...
    try {
      ...
      setBulkRejectionReason(''); // Clear bulk rejection reason
      fetchPendingSubmissions();
      fetchSubmissions();
    } catch (error) {
          ...
          <div className="space-y-4">
            {selectedSubmissions.size > 0 && (
              <div className="flex gap-2 mb-4">
                ...
                <NeuroButton onClick={() => bulkUpdateSubmissions('reject')} variant="secondary" size="sm">
